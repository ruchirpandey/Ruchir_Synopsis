...
This chapter presents the implementation of DDPG \cite{Lillicrap2015} and TD3 \cite{Fujimoto2018} algorithms for unified control of the hybrid DFIG-Solar PV system. The theoretical foundations of deep reinforcement learning---including the MDP formulation, policy gradient methods, actor-critic architecture, experience replay, and target networks---are detailed in Chapter~\ref{chap:rl}. Here, the focus is on the proposed control framework design and algorithm-specific implementation decisions.

The RL agent-environment interaction for the DFIG-PV system is illustrated in Figure~\ref{fig:rl_framework}, where the agent (DDPG or TD3 controller) observes system states and outputs converter voltage references, receiving reward feedback based on power tracking and voltage regulation performance.
...
\begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{Agent-environment interaction in reinforcement learning for DFIG-PV control}
...
\subsubsection{Emergency Fallback Strategy}

During both training and HIL deployment, a safety monitor continuously checks the DRL controller outputs against physical constraints. If any action violates converter voltage limits ($|v| > v_{max}$), DC link voltage deviates beyond $\pm$10\% of nominal, or the reward drops below a critical threshold for 10 consecutive steps, control is immediately transferred to a conventional PI controller with pre-tuned gains. This fallback mechanism ensures system safety during RL exploration and provides a fail-safe for unexpected operating conditions during deployment.

\subsection{Summary of Common Elements}

These elements---state/action spaces, dynamics, reward, networks, infrastructure, normalization, and safety mechanisms---form the unified foundation for DDPG and TD3.
...
The quantitative performance comparison based on OPAL-RT HIL simulation results is presented in Chapter 5.