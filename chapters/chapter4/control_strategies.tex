...
System state evolution governed by nonlinear differential equations (Chapter~\ref{chap:model}):
...
This is a Multi-objective quadratic reward function that balances power tracking, voltage regulation, and stability.
...
Architecture: It consists of input layer of [11] neurons with [400x300] hidden layers and output layers with [4] neurons with ReLU activation and tanh output.
...
Architecture: It consists of input layer of [15] neurons with [400x300] hidden layers and output layers with [1] neurons with ReLU activation and linear output. DDPG uses 1 critic, TD3 uses 2 (Section~\ref{subsec:td3_critics}).
...
\begin{itemize}
\item \textbf{Software:} Python 3.12, NumPy 1.23.0, MATLAB R2019b (HIL).

\item \textbf{Hardware:} Google Colab Pro was used which avails NVIDIA Tesla T4, 16 GB VRAM for training ML models and for real time testing OPAL-RT OP4510 HIL was used.
\end{itemize}
...
Gradual task difficulty increase in following phases: 
\begin{itemize}

\item \textbf{Phase 1 (Episode 1--100):} Random exploration, buffer population. 
\item \textbf{Phase 2 (Episode 101--500):} DC link voltage only ($r = -(v_{dc} - v_{dc}^*)^2$). 
\item \textbf{Phase 3 (Episode 501--1000):} DC link + RSC power ($r = r_{RSC} + r_{GSC,voltage}$). 
\item \textbf{Phase 4 (Episode 1001+):} Full multi-objective (Eq.~\ref{eq:total_reward}) is essential for convergence.
\end{itemize}
...
The actor-critic network for continuous control with deterministic policy gradients has been discussed in DDPG algorithm for hybrid DFIG-PV system \cite{Pandey2025DDPG}.

\subsubsection{DDPG Overview}
\label{subsec:ddpg_overview}

It extends DQN to deterministic policy with simplified gradient, off-policy learning with replay buffer and sample efficiency, actor-critic where actor proposes and critic evaluates continuous action space with direct voltage commands and fine-grained control.

\subsubsection{Single Critic Architecture}
\label{subsec:ddpg_critic}

Single critic $Q_\phi(s,a)$ estimates action-value: $Q^\pi(s,a) = E\left[ \sum_{t=0}^{\infty} \gamma^t r_t \mid s_0=s, a_0=a, \pi \right]$ as discussed in Section~\ref{subsec:nn_foundation}.
...
\item[]  \textbf{Learning:} Discount factor $\gamma = 0.99$, actor learning rate $1 \times 10^{-4}$, critic learning rate $1 \times 10^{-3}$, target update rate $\tau = 0.001$.

\item[]  \textbf{Training:} Batch size 64, replay buffer $1 \times 10^6$, Ornstein-Uhlenbeck noise ($\theta = 0.15$, $\sigma = 0.2$).
\end{itemize}
\subsubsection{DDPG Reward Weights}
\label{subsec:ddpg_rewards}
\begin{itemize} 
\item[] \textbf{RSC weights:} $w_1 = 0.4$, $w_2 = 0.3$, $w_3 = 0.3$.

\item[] \textbf{GSC weights:} $w_4 = 0.3$, $w_5 = 0.2$, $w_6 = 0.2$.
\end{itemize}
\subsubsection{DDPG Training Configuration}
\label{subsec:ddpg_training_config}
\begin{itemize} 
\item[] \textbf{Episodes:} 2000 episodes × 1000 steps each.

\item[] \textbf{Curriculum:} Warmup (episodes 1--100), simplified task (101--500), intermediate (501--1000), full multi-objective (1001--2000).

\item[] \textbf{Exploration:} Minimum noise 0.1, decay factor 0.995 per episode.

\item[] \textbf{Convergence:} Same criterion as Eq.~\ref{eq:convergence_criterion} with 100 consecutive episodes (less stringent than TD3). Total training time $\approx$8 hours.
\end{itemize}
...
\begin{enumerate}
    ...
    \item Add immediate reward: $y_i = r_i + \gamma \min(Q'_{\phi_1}, Q'_{\phi_2})$
\end{enumerate}
...
Only the first critic $Q_{\phi_1}$ is used for the policy gradient, as using both would double the gradient computation cost without significant benefit.
...
\item[] \textbf{Learning:} Discount factor $\gamma = 0.99$, actor learning rate $8 \times 10^{-5}$, critic learning rate $7.5 \times 10^{-4}$, target update rate $\tau = 0.0008$.

\item[] \textbf{Training:} Batch size 64, replay buffer $1 \times 10^6$.

\item[] \textbf{TD3-Specific:} Policy delay $d = 2$, target policy noise $\tilde{\sigma} = 0.2$, noise clip $c = 0.5$, exploration noise $\sigma_{explore} = 0.1$.
\end{itemize}
...
\item[] \textbf{GSC weights:} $w_4 = 0.2$, $w_5 = 0.4$, $w_6 = 0.4$.
\end{itemize}
\subsubsection{TD3 Training Configuration}
\label{subsec:td3_training_config}
\begin{itemize}

\item[] \textbf{Episodes:} 2500 episodes × 1600 steps each.

\item[] \textbf{Curriculum:} Warmup (episodes 1--100), simplified task (101--300), intermediate (301--800), full multi-objective (801--2500).

\item[] \textbf{Exploration Decay:} TD3 uses Gaussian exploration noise with linear decay: $\sigma_{explore}(e) = \max(0.01, \sigma_0 - e \times \Delta\sigma)$, where $\sigma_0 = 0.1$ and $\Delta\sigma = 4 \times 10^{-5}$ per episode, reaching minimum noise at approximately episode 2250.

\item[] \textbf{Convergence Criterion:} Training is considered converged when the 100-episode moving average of the total reward $\bar{R}_{100}$ satisfies:
\begin{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Low reward variance:} The stable reward trajectory demonstrates that target policy smoothing effectively regularises the policy.
...
\subsection{TD3 based Learning Study Methodology}
...
The DDPG  and TD3 has been compared for architectural differences, computational trade-offs and deployment considerations.
...
Based on studies conducted during development:
...
\end{enumerate}
 ...
 For the DFIG-Solar PV hybrid system, \textbf{TD3 is the preferred choice} for production deployment despite longer training time, due to its superior stability, robustness, and safety characteristics. DDPG remains valuable for rapid prototyping and initial development.

The quantitative performance comparison based on OPAL-RT HIL simulation results is presented in Chapter~\ref{chap:experiments}.