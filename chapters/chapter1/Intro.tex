...
The global energy landscape is undergoing a fundamental change driven by the urgent need to mitigate climate change, decarbonize the energy systems, and reduce reliance on fossil fuels. Renewable generation particularly wind and solar photovoltaic (PV) systems—has become central to this transition. Recent reports from international agencies project that renewable electricity generation will continue its strong growth with solar PV and wind contributing to the majority of global capacity expansion \cite{Uddin2023,Khare2023}. These technologies offer significant environmental and economic benefits but also introduce operational challenges arising from the intermittent nature.

Unlike conventional Power plants with synchronous machines, which naturally provide rotational inertia and predictable electromechanical behavior, renewable energy sources introduce high variability and limited controllability. Wind speed may change within seconds, while solar irradiance fluctuates due to cloud covers and diurnal patterns. Such variability leads to operational uncertainty, complicating the tasks of maintaining frequency stability, voltage regulation and power quality challenges.  These are documented in recent microgrid and renewable integration studies \cite{Ahmed2023,Shezan2023,Smith2022GridStability}. Recent reports confirm that the integration of inverter-based renewable energy sources leads to significant reduction in system inertia, increasing the rate of change of frequency (RoCoF) and degrading traditional frequency control efficacy \cite{Smahi2025,He2024}.

There are challenges to design advanced control architectures that not only accommodate but optimally exploit the characteristics of hybrid, variable renewable energy sources while maintaining grid stability.

\section{Renewable Energy Integration Challenges}

\subsection{Intermittency and Variability}

There are challenges associated with renewable energy integration that stems from the stochastic and non-stationary nature of wind and solar resources. Wind turbines can experience power output swings from near-zero to rated power within minutes, while photovoltaic (PV) arrays exhibit both predictable diurnal cycles and abrupt short-term fluctuations \cite{Uddin2023}. Although wind and solar are often described as complementary—wind power tends to peak at night or during cloudy conditions—the extent of this complementarity is strongly site-dependent and seasonal and is well described in performance comparison studies of hybrid PV–wind microgrids \cite{Tang2021_PVwind,Prasad2025}.

It is interesting to understand whether complementary wind and solar resources genuinely mitigate intermittency, or they introduce new multi-timescale uncertainty and control coupling.

Recent literature suggests that complementarity often reduces net variability but does not eliminate it. It creates a rich but more complex control environment that requires sophisticated coordination strategies \cite{Shezan2023,MicrogridFrequencyStability2024}.

\subsection{Grid Stability Requirements}

Modern power systems must satisfy tight operational constraints to ensure secure and reliable operation. Frequency stability must be maintained within $\pm 0.2$~Hz of nominal frequency, while voltage regulation must remain within $\pm 5\%$ of nominal levels. The renewable sources must provide inertia support through virtual inertia mechanisms to compensate for reduced rotational mass in converter-dominated grids.

Traditional control strategies, primarily linear controllers such as PI were designed for systems with predictable dynamics and high inertia. Their performance degrades beyond a certain range when applied to modern converter-dominated systems with fast dynamics and significant coupling, particularly in hybrid renewable energy systems \cite{Halivor2023_MPC}. Therefore there is requirement for an adaptive, model predictive, and intelligent control mechanisms to address the increasing complexity of renewable-rich grids \cite{Joshal2023,Kamal2022}.
...
\end{enumerate}
 ...
 While hardware simplification is advantageous, recent research shows that hybrid architectures may introduce additional dynamic interactions, requiring more sophisticated control strategies to take advantages of performance benefits \cite{Nguyen2023_DFIG,Tang2021_PVwind,Bhattacharyya2022}. The tightly coupled DC-link dynamics requires coordinated control approaches that can simultaneously manage wind power extraction, solar MPPT, and grid power exchange while maintaining voltage stability.
...
The control challenge is further complicated by the multi-timescale dynamics where wind variations occur over seconds to minutes while solar fluctuations range from fraction of second to minutes \cite{Prasad2025}, operational mode transitions requiring switching between sub-synchronous and super-synchronous operation \cite{Akhbari2023_DFIG}, and grid code compliance demanding simultaneous satisfaction of fault ride-through, voltage support, and frequency regulation requirements \cite{Bhattacharyya2022}.

These multi objective control challenges have motivated increasing attention toward reinforcement learning and advanced predictive controllers as viable alternatives to conventional control architectures.
...
The application of artificial intelligence and machine learning to control  power system  has evolved rapidly over the past decade. Early approaches relied on rule-based fuzzy logic and traditional neural networks, which had improvement upon fixed-gain controllers. They lacked the ability to learn optimal policies directly from system interactions and are tuned for a specific model. They are also not suitable for similar model of different ratings\cite{Uddin2023}.

Deep reinforcement learning (DRL) enables controllers to learn optimal decision-making policies through trial-and-error interaction with the system environment. Unlike supervised learning approaches that require labeled training data, DRL agents learn from reward signals based on control objectives. These are  particularly suitable for complex, nonlinear systems where analytical solutions are intractable.
...
Value Estimation & Prone to overestimation & Conservative estimation via minimization\\
...
{The work in this thesis directly addresses this gap by developing, implementing, and validating TD3-based control for DFIG-Solar PV hybrid systems, addressing both DRL applications and renewable energy integration.}

\section{Research Objectives}

The scope of this work address the control challenges of hybrid DFIG–PV systems through advanced deep reinforcement learning (DRL) methodologies. The primary objective is to design, implement and evaluate DRL-based controllers capable of learning optimal control policies under varying, coupled and nonlinear operating conditions.

\textbf{Primary Objective:} To develop, implement, and experimentally validate DRL-based control strategies (DDPG and TD3) that achieve superior dynamic performance compared to classical PI-based methods in solar PV–integrated DFIG wind energy systems.

\textbf{Detailed Objectives:}
\begin{enumerate}
    \item \textbf{Theoretical Development:} Formulate detailed mathematical models of the integrated hybrid DFIG-PV system incorporating DC-link coupling dynamics. Construct comprehensive state and action representations that capture critical system behaviors including power electronic dynamics. Develop multi-objective reward functions balancing competing objectives: power tracking accuracy, DC-link voltage stability, and system efficiency.

    \item \textbf{Algorithm Implementation:} Implement both both DDPG \cite{Lillicrap2015} and TD3 \cite{Fujimoto2018} algorithms suited for continuous control of electronic power converters. Address DDPG limitations through TD3 enhancements: clipped double Q-learning, delayed policy updates, and target policy smoothing. \cite{Zholtayev2024}. Develop curriculum learning strategies to facilitate policy convergence for multi-objective control tasks. Integrate physics-based constraints to ensure safe operation, building upon recent work in constrained DRL \cite{PhysicsConstrainedTD32024}.

    \item \textbf{Performance Validation:} Conduct Hardware-in-the-Loop (HIL) experiments using the OPAL-RT platform \cite{Prabakar2019_NREL,Prabakar2020_NREL,vonJouanne2023,Pandey2022PIICON}. Design comprehensive test scenarios covering: steady-state operation, transient disturbances, and variable renewable inputs. Compare DRL controllers (both DDPG and TD3) with classical PI under identical test conditions and performance metrics. Quantify improvements in: settling time, overshoot, response speed and DC-link voltage regulation.

    \item \textbf{Critical Analysis and Algorithmic Comparison:} Provide detailed comparative analysis of DDPG vs. TD3 performance in the specific context of DFIG-PV hybrid systems. Evaluate computational complexity and real-time feasibility for embedded deployment. Identify operating scenarios where each DRL algorithm demonstrates marked superiority or limitations.

    \item \textbf{Practical Hardware Contributions:} Demonstrate practical feasibility through OPAL-RT HIL validation, bridging the gap between simulation and real-world deployment. Contribute to the growing body of knowledge on DRL applications in renewable energy systems \cite{Smahi2025,He2024}.
\end{enumerate}

\textbf{Novel Contributions:}

This research makes following contributions to the field:
\begin{enumerate}
    \item \textbf{Application of TD3 to DFIG-Solar PV hybrid systems} with direct DC-link integration, addressing a critical gap in existing literature.
    \item \textbf{Detailed DDPG-TD3 comparative study} in the context of multi-source renewable systems, providing insights.
    \item \textbf{Unified controller design} managing RSC, GSC, and PV MPPT simultaneously through a single DRL agent.    
    \item \textbf{Performance benchmarking} against conventional PI controller \cite{GridIntegratedDFIG2025}.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Chapter~\ref{chap:literature}} provides a survey of relevant literature spanning: hybrid renewable energy systems, DFIG control strategies, solar PV integration techniques, deep reinforcement learning fundamentals, and recent applications of DRL in power systems with emphasis on 2023-2025 developments.
    
    \item \textbf{Chapter~\ref{chap:model}} details the comprehensive system modeling framework including: DFIG electromagnetic and mechanical models, solar PV single-diode equivalent circuit, DC-link dynamics, power converter models (RSC and GSC), and grid interface representation.
    
       
    \item \textbf{Chapter~\ref{chap:ddpg_implementation}} describes DDPG methodology including: environment formulation, state-action space design, reward function development, neural network architectures, training procedures. It also presents the TD3 algorithm including: algorithmic enhancements over DDPG, implementation details, hyperparameter optimization, curriculum learning strategies, and comparative training analysis.
    
    \item \textbf{Chapter~\ref{chap:experiments}} outlines the comprehensive HIL experimental setup including: OPAL-RT platform specifications, system parameter configuration, test scenario design and validation methodology aligned with industry standards \cite{Prabakar2019_NREL,vonJouanne2023}.    
      It also provides detailed performance analysis including: steady-state operation results, transient response characterization, DDPG vs. TD3 comparative metrics and benchmarking against PI control baselines. It also discusses experimental findings including interpretation of performance differences, analysis of algorithm strengths and limitations, practical deployment considerations, computational feasibility for real-time control, and results and expectations\cite{Smahi2025,He2024,Liang2024}.
    
    \item \textbf{Chapter~\ref{chap:conclusions}} discusses the results and give conclude the summary of key findings, novel contributions to knowledge, practical implications for industry, limitations of the current work, and detailed recommendations for future research directions including multi-agent extensions and integration with physics-informed machine learning \cite{WindStoragePINN2024}.
\end{itemize}

\textbf{Acknowledgment of Supporting Research Work:}
