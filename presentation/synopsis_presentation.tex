...
% Title information
\title[TD3 for DFIG-PV Systems]{Twin-Delayed Deep Deterministic Policy Gradient for Enhanced Power Optimization in Solar PV-Integrated DFIG Wind Energy Systems}
\subtitle{Ph.D. Synopsis Presentation}
\author[Ruchir Pandey]{Ruchir Pandey\\[0.3cm]{\small Supervisor: Dr. Shantanu Bose}}
\institute[NIT Uttarakhand]{Department of Electrical Engineering\\National Institute of Technology, Uttarakhand}
\date{2025}
...
\begin{itemize}
    ...
    \item \textbf{Stringent grid code requirements} (THD $<$ 5\%, voltage $\pm$5\%)
\end{itemize}
\end{alertblock}
...
No comprehensive study applies \textbf{TD3} to integrated \textbf{DFIG-Solar PV} hybrid systems with:
\begin{itemize}
...
\textit{How can we design DRL-based control that optimally exploits hybrid DFIG-PV characteristics while maintaining grid stability?}
\end{block}
\end{frame}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Implement DDPG and TD3 for continuous converter control
        \item Optimize neural network architectures
        \item Develop curriculum learning strategies
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Identify operating scenarios for each algorithm
    \end{itemize}
\end{enumerate}
\end{frame}
...
\textbf{Advantages:}
\begin{itemize}
    \item Shared power electronics
    \item 15-25\% cost reduction
    \item 3-5\% efficiency improvement
    \item Reduced component count
\end{itemize}

\column{0.5\textwidth}
\begin{block}{Control Objectives}
\begin{itemize}
    \item \textbf{RSC:} Rotor current control, torque regulation, MPPT
    \item \textbf{GSC:} DC-link voltage, grid power exchange
    \item \textbf{PV:} Maximum power extraction
\end{itemize}
\end{block}

\begin{alertblock}{Coupling Challenge}
Disturbance in any subsystem propagates to others via shared DC-link dynamics
\end{alertblock}
\end{columns}
\end{frame}
...
More stable training, better convergence, and improved deployment performance
\end{alertblock}
\end{frame}
...
\begin{columns}
\column{0.5\textwidth}
\textbf{OPAL-RT OP4510 Platform:}
\begin{itemize}
    \item Real-time digital simulator
    \item Sub-microsecond time steps
    \item Industry-standard validation
    \item Realistic power system dynamics
\end{itemize}
...
\begin{itemize}
    ...
    \item Solar irradiance steps (0--50 A)
...
\vspace{0.2cm}
Initial conditions: Wind = 9 m/s, steady-state
\end{columns}
\end{frame}
...
% ============== SLIDE 13: Rotor Current Results ==============
...
\begin{columns}
\column{0.5\textwidth}
\textbf{Grid Power Transformation:}

\vspace{0.3cm}
\begin{tabular}{|l|c|c|}
\hline
\textbf{Parameter} & \textbf{No Solar} & \textbf{With Solar} \\
\hline
$I_{solar}$ & 0 A & 50 A \\
$P_{solar}$ & 0 kW & 11.5 kW \\
$P_{grid}$ & -8 to -5 kW & +3 to +6 kW \\
\hline
\end{tabular}

\vspace{0.3cm}
Negative-to-positive transition indicates transformation from \textbf{grid import} to \textbf{grid export} mode.
...
\begin{enumerate}
    \item \textbf{First TD3 Application to DFIG-PV Hybrid Systems}
    \begin{itemize}
        \item Novel application of TD3 to integrated solar PV-DFIG systems
        \item Addresses unique coupling dynamics in hybrid configurations
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Demonstrates industrial deployment feasibility
    \end{itemize}
\end{enumerate}
\end{frame}
...
\begin{itemize}
    ...
    \item TD3 provides more \textbf{stable training} and \textbf{balanced performance}
    \item Solar PV integration improves DC-link stability by 16-17\%
    \item HIL validation confirms practical deployment viability
\end{itemize}
\end{block}
...
\begin{itemize}
    \item TD3 has higher training complexity but better deployment reliability
...
\begin{itemize}
    \item Multi-agent DRL for wind farms
    \item Physics-informed neural networks
    \item Transfer learning across conditions
    \item Fault ride-through capability
\end{itemize}

\vspace{0.3cm}
\textbf{System Extensions:}
\begin{itemize}
    \item Battery energy storage integration
    \item Grid fault response
    \item Microgrid coordination
\end{itemize}

\column{0.5\textwidth}
\textbf{Deployment Considerations:}
\begin{itemize}
    \item Embedded implementation
    \item Real-time computational optimization
    \item Field testing and validation
    \item Safety-constrained learning
\end{itemize}

\vspace{0.3cm}
\textbf{Scalability:}
\begin{itemize}
    \item Large-scale wind-solar farms
    \item Grid-level coordination
...
{\small National Institute of Technology, Uttarakhand}

\vspace{0.5cm}
{\small Supervisor: Dr. Shantanu Bose}
\end{center}
\end{frame}
